...
p.plt <- function(x, SD = F){ if (isFALSE(SD)){ # Return Plot
    ...
    if (x[nrow(x),] > 0){ plot(x, main = "Plot of Portfolio Returns", 
                               type="l", las = 1, xlab = "Trading Days",
                               lwd = 3, ylab = "Return (%)", col = "green4")
      
    } else { plot(x, main = "Plot of Portfolio Returns", xlab = "Trading Days", 
                  ylab="Return (%)", type="l", las = 1, col = "red3",lwd = 3) }
    
    par(mar = c(5, 5, 5, 5)) # Define borders of the plot
    
    axis(side = 2, las = 1, at = seq(-100, 100, 1)) # Axes
    axis(side = 4, las = 1, at = seq(-100, 100, 1))
