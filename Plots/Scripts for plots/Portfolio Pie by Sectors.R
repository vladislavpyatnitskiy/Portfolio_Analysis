...
p.pie.plt.sector <- function(x, main = NULL, sub = NULL){ 
  ...
  tickers <- colnames((x[,1 + 3 * seq(31, from = 0)]))  # Subset tickers
  
  pct <- as.data.frame(x[,3+3*seq(31,from=0)])/x[,ncol(x)] # Calculate portions
  ...
  # Pie Chart
  pie(pie.df[,2],labels=c(sprintf("%s %s%%",pie.df[,1],pie.df[,2])),
      col=colors37, main=main,sub=sub, radius=1.5) 
}
