...
p.pie.plt.marketcap <- function(x){ # Portfolio Pie Plot by Market Cap
  ...
  y <- round(as.data.frame(t(as.data.frame(x[,3+3*seq(31,from=0)][nrow(x),]))/
                             x[nrow(x),][,ncol(x)]),2)*100
  rownames(y) <- tickers
  ...
  pie(df[,2], labels = c(sprintf("%s %s%%", df[,1], df[,2])), col = colors37,
      main = "Portfolio by Market Capitalisation", radius = 1.1) # Plot
}
