...
p.bar.plt.stack.dividend.country <- function(x, portion = F){ 
  ...
  colnames(y) <- "Country" # Assign Column Name (Country)
  ...
  rownames(l) <- l[,1] # Assign Country info as row names
  
  l <- l[,-1] # Reduce Country info as it is now in row names
  ...
  if (isTRUE(portion)){ # Plot showing stakes of dividends for each month
    ...
    ggplot(i, aes(x = Date, y = Quantity, fill = Stock)) + theme_minimal() +
      ...
      labs(title="Stacked Bar Plot of Portfolio Dividends by Country (%)",
           x = "Months", y = "Stakes (%)", fill = "Country") 
    ...
    ggplot(i, aes(x = Date, y = Quantity, fill = Stock)) + theme_minimal() +
      ...
      labs(title="Stacked Bar Plot of Portfolio Dividends by Country ($)",
           x = "Months", y = "Amount in $US", fill = "Country") } 
}
p.bar.plt.stack.dividend.country(df_portfolio_dividend, portion = F) # Test
